  ...
  <script src="http://code.jquery.com/jquery-latest.min.js"></script>
	...
	<script type="text/javascript">
			 ...
			 $('#summernote').summernote({
			  ...
			  toolbar: [
				 ...
				 callbacks: { //여기 부분이 이미지를 첨부하는 부분
					  ...
					  //하단 함수 호출 
						  uploadImage(files[0],this); //이미지 업로드 호출 - 이미지파일,현재위치 값
					  },
					  onPaste: function (e) {
		...
		function uploadImage(file,this_location){ //매개변수 -file:이미지파일, 현재위치값
			var fdata = new FormData(); //ajax에서 전송 할때 form 형태로 전송
			fdata.append("file",file);//file 이미지를 첨뷰
			$.ajax({
				url:"/uploadImage",
				type:"post",
				dataType:"text", //파일을 받는 형태 -text,json,xml.....
				data:fdata,
				cache:false, //남아있는 데이터 정보 새롭게 정리
				contentType:false, // 보내는형태 default값 : application/x-www-form-urlencoded
				processData:false, //query String 형태로 보내지 않음. http://localhost:8000/bno=1&
				enctype:"multipart/form-data",
				success:function(data){ //서버에가서 이미지를 저장후 파일이름을 리턴 
					//data:/upload/4857948_1.jpg
					$(this_location).summernote('editor.insertImage',data); //js에서 함수호출해서 위치값에 데이터를넣어줌
